...
<?php
  ...
  include 'funcoes.php';
  $titulo = 'Produtos';
  $link = 'produtos.php#tabela';
  $tabela = $GLOBAL['tbProdutos'];
  $pesquisa = isset($_POST['pesquisa'])?$_POST['pesquisa']:'';
  $filtro = isset($_POST['filtro'])?$_POST['filtro']:'nenhum';
  if ($filtro == 'codigoProduto' || $filtro == 'nenhum') {
    $aux = '';
    if ($pesquisa != '') { $aux = ' where '. $filtro.' = '. $pesquisa. ' order by '.$filtro; }
    $sql = 'SELECT * FROM '. $tabela. $aux;
  } elseif ($filtro == 'descricao' || $filtro == 'valor' || $filtro === 'codigoDeBarra') {
...
<body>
    ...
    <header>
        <ul id="nav-mobile" class="sidenav">
            <li><a href="index.php">Marcas</a></li>
                        ...
                        <div class="row">
                            ...
                            <div class="input-field col m6 s12 l4">
                                <select name="filtro">
                                    ...
                                    <option value="codigoProduto" <?php echo $filtro == 'codigoProduto' ? 'selected ' : '' ; ?>>Código</option>
                                    <option value="descricao" <?php echo $filtro == 'descricao' ? 'selected ' : '' ; ?>>Descricao</option>
                                    <option value="valor" <?php echo $filtro == 'valor' ? 'selected ' : '' ; ?>>Valor</option>
                                    <option value="codigoDeBarra" <?php echo $filtro == 'codigoDeBarra' ? 'selected ' : '' ; ?>>Código de Barras</option>
                                </select>
                                <label>Filtrar</label>
                            </div>
                        ...
                        <div class="input-field col s12">
                            <table class="striped responsive-table centered">
                                ...
                                <tbody>
                                    ...
                                    <tr>
                                        <td>
                                            <?php echo $tupla['codigoProduto']; ?>
                                        </td>
                                        <td>
                                            <?php echo $tupla['descricao']; ?>
                                        </td>
                                        <td>$
                                            <?php echo number_format($tupla['valor'], 2, ',', '.'); ?>
                                        </td>
                                        <td>
                                            <?php echo $tupla['codigoDeBarra']; ?>
                                        </td>
                                        <td>
                                            <?php echo Query1paraN($GLOBAL['tbMarcas'], 'codigo', $tupla['marca'], 'nome') ?>
                                        </td>
                                        <!-- Alterar -->
                                        <td>
                                          <a class="waves-effect waves-light modal-trigger green-text" href="#alterar<?php echo $tupla['codigoProduto'] ;?>"><i class="material-icons">build</i></a>
                                          <div id="alterar<?php echo $tupla['codigoProduto'] ;?>" class="modal">
                                              <div class="modal-content">
                                                  <center>
                                                      <h4>Alterar</h4>
                                                      <form action="acao.php" method="post">
                                                          <div class="input-field col s12">
                                                              <input id="descricao" type="text" name="descricao" value="<?php echo $tupla['descricao']; ?>" />
                                                              <label for="descricao">Descrição</label>
                                                          </div>
                                                          <div class="input-field col s12">
                                                              <input id="valor" type="number" name="valor" value="<?php echo $tupla['valor']; ?>" step="0.01" />
                                                              <label for="valor">Valor</label>
                                                          </div>
                                                          <div class="input-field col s12">
                                                              <input id="codigoDeBarra" type="text" name="codigoDeBarra" value="<?php echo $tupla['codigoDeBarra']; ?>" />
                                                              <label for="codigoDeBarra">Código de barras</label>
                                                          </div>
                                                          <div class="input-field col s12">
                                                            <?php geradorSelect($GLOBAL['tbMarcas'], $tupla['marca'], 'codigo', 'nome', 'marca') ?>
                                                          </div>
                                                          <center>
                                                              <input type="hidden" name="tabela" value="<?php echo $tabela; ?>">
                                                              <input type="hidden" name="link" value="<?php echo $link; ?>">
                                                              <input type="hidden" name="codigoProduto" value="<?php echo $tupla['codigoProduto']; ?>">
                                        ...
                                        <!-- Excluir -->
                                        <td>
                                            <a class="waves-effect waves-light modal-trigger red-text" href="#excluir<?php echo $tupla['codigoProduto']; ?>"><i class="material-icons">clear</i></a>
                                            <div id="excluir<?php echo $tupla['codigoProduto']; ?>" class="modal" style="width: 350px;">
                                                <div class="modal-content">
                                                    <center>
                                                        <h4>Confirmar Exclusão!</h4>
                                                        <p>Deseja realmente excluir esse registro?<br />Essa operação não poderá ser desefeita</p>
                                                        <a href="#!" class="modal-action modal-close waves-effect waves-red btn-flat">Cancelar</a>
                                                        <a href="acao.php?acao=excluir&codigo=<?php echo $tupla['codigoProduto']; ?>&tabela=<?php echo $tabela; ?>&link=<?php echo $link; ?>" class="modal-action waves-effect waves-teal btn-flat">Confirmar</a>
                                                    </center>
                                                </div>
                                            </div>
                                        </td>
                                            ...
                                            <div class="modal-content">
                                                <center>
                                                    ...
                                                    <form action="acao.php" method="post">
                                                        ...
                                                        <div class="input-field col s12">
                                                            <input id="codigoDeBarra" type="text" name="codigoDeBarra" />
                                                            <label for="codigoDeBarra">Código de barras</label>
                                                        </div>
                                                        <div class="input-field col s12">
                                                          <?php geradorSelect($GLOBAL['tbMarcas'], '1', 'codigo', 'nome', 'marca') ?>
                                                        </div>
                                                        <center>
                                                            <input type="hidden" name="tabela" value="<?php echo $tabela; ?>">
                                                            <input type="hidden" name="link" value="<?php echo $link; ?>">
